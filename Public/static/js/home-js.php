...
function changeLang(tthis) {
  ...
  switch (lang) {
    case "en-US":
       ...
       break;
    case "it-IT":
       window.open("https://blog-gaoxio-com.translate.goog/<?PHP echo($AVATAR_NAME);?>?_x_tr_sl=auto&_x_tr_tl=it&_x_tr_hl=en&_x_tr_pto=wapp","_self");
       break;
    case "zh-CN":
       ...
       window.open("http://webtrans.yodao.com/webTransPc/index.html?from=en&to=zh-CHS&type=1&url=https%3A%2F%2Fblog.gaoxio.com%2F<?PHP echo($AVATAR_NAME);?>", "_self");
       break;
   }    
...
$("div.dragover").on("drop", function(e) {
  ...
  if (count !== 0) {
    ...
    $.ajax("/<?PHP echo($AVATAR_NAME);?>/detail", {
        method: "POST",
        processData: false,
        contentType: false,
        data: fd//,
